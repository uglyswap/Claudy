...
// Opens a separate console window for clean input (avoids Ink terminal conflicts)
// IMPORTANT: Must return text in value field, console.log doesn't display in Claude Code
const cleApiInline = '{type:"local",name:"cle-api",description:"Changer la cle API Z.AI",isEnabled:()=>!0,isHidden:!1,supportsNonInteractive:!1,async call(input){try{const f=await import("fs"),p=await import("path"),o=await import("os"),cp=await import("child_process");let K=String(input||"").trim();if(!K){const tmp=p.join(o.tmpdir(),"claudy-key-"+Date.now()+".txt");const ps="Add-Type -AssemblyName System.Windows.Forms; [System.Windows.Forms.Application]::EnableVisualStyles(); $form = New-Object System.Windows.Forms.Form; $form.Text = \\\"Claudy - Nouvelle cle API Z.AI\\\"; $form.Size = New-Object System.Drawing.Size(500,150); $form.StartPosition = \\\"CenterScreen\\\"; $form.FormBorderStyle = \\\"FixedDialog\\\"; $form.MaximizeBox = $false; $label = New-Object System.Windows.Forms.Label; $label.Location = New-Object System.Drawing.Point(10,20); $label.Size = New-Object System.Drawing.Size(460,20); $label.Text = \\\"Entrez votre nouvelle cle API Z.AI:\\\"; $form.Controls.Add($label); $textBox = New-Object System.Windows.Forms.TextBox; $textBox.Location = New-Object System.Drawing.Point(10,50); $textBox.Size = New-Object System.Drawing.Size(460,20); $form.Controls.Add($textBox); $button = New-Object System.Windows.Forms.Button; $button.Location = New-Object System.Drawing.Point(200,80); $button.Size = New-Object System.Drawing.Size(75,23); $button.Text = \\\"OK\\\"; $button.DialogResult = [System.Windows.Forms.DialogResult]::OK; $form.AcceptButton = $button; $form.Controls.Add($button); $form.Topmost = $true; $form.Add_Shown({$textBox.Select()}); $result = $form.ShowDialog(); if($result -eq [System.Windows.Forms.DialogResult]::OK){$textBox.Text | Out-File -FilePath \\\""+tmp.replace(/\\\\/g,"\\\\\\\\")+"\\\"}";cp.execSync("powershell -NoProfile -Command \\""+ps+"\\"",{windowsHide:true});if(f.existsSync(tmp)){K=f.readFileSync(tmp,"utf8").trim();f.unlinkSync(tmp)}if(!K){return{type:"text",value:"Annule - aucune cle fournie."}}}const sp=p.join(o.homedir(),".claudy","settings.json");const c=f.readFileSync(sp,"utf8");const s=JSON.parse(c);const ok=s.env&&s.env.ANTHROPIC_AUTH_TOKEN;if(!ok)throw new Error("Ancienne cle non trouvee");const cnt=(c.match(new RegExp(ok.replace(/[.*+?^${}()|[\\]\\\\]/g,"\\\\$&"),"g"))||[]).length;const nc=c.split(ok).join(K);f.writeFileSync(sp,nc,"utf8");const m1=ok.length>10?ok.substring(0,6)+"..."+ok.slice(-4):"***";const m2=K.length>10?K.substring(0,6)+"..."+K.slice(-4):"***";return{type:"text",value:"\\n========================================\\n   CLE API Z.AI MISE A JOUR\\n========================================\\n\\nAncienne: "+m1+"\\nNouvelle: "+m2+"\\n\\n- ANTHROPIC_AUTH_TOKEN: OK\\n- Z_AI_API_KEY (vision): OK\\n- Authorization web-search-prime: OK\\n- Authorization web-reader: OK\\n\\n"+cnt+" occurrence(s) remplacee(s)\\n\\nRedemarrez Claudy pour appliquer.\\n"}}catch(e){return{type:"text",value:"[ERREUR] "+e.message}}},userFacingName(){return"cle-api"}}';

const cleInline = '{type:"local",name:"cle",description:"Alias pour /cle-api",isEnabled:()=>!0,isHidden:!0,supportsNonInteractive:!1,async call(input){try{const f=await import("fs"),p=await import("path"),o=await import("os"),cp=await import("child_process");let K=String(input||"").trim();if(!K){const tmp=p.join(o.tmpdir(),"claudy-key-"+Date.now()+".txt");const ps="$k = [Microsoft.VisualBasic.Interaction]::InputBox(\\\"Entrez votre nouvelle cle API Z.AI:\\\",\\\"Claudy\\\",\\\"\\\"); if($k){$k | Out-File -FilePath \\\""+tmp.replace(/\\\\/g,"\\\\\\\\")+"\\\"}";cp.execSync("powershell -NoProfile -Command \\"Add-Type -AssemblyName Microsoft.VisualBasic; "+ps+"\\"",{windowsHide:true});if(f.existsSync(tmp)){K=f.readFileSync(tmp,"utf8").trim();f.unlinkSync(tmp)}if(!K){return{type:"text",value:"Annule."}}}const sp=p.join(o.homedir(),".claudy","settings.json");const c=f.readFileSync(sp,"utf8");const s=JSON.parse(c);const ok=s.env&&s.env.ANTHROPIC_AUTH_TOKEN;if(!ok)throw new Error("Ancienne cle non trouvee");const nc=c.split(ok).join(K);f.writeFileSync(sp,nc,"utf8");return{type:"text",value:"[OK] Cle API mise a jour. Redemarrez Claudy."}}catch(e){return{type:"text",value:"[ERREUR] "+e.message}}},userFacingName(){return"cle"}}';
